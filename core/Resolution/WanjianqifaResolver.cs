...
/// </summary>
public sealed class WanjianqifaResolver : UntargetedMassTrickResolverBase
{
    ...
    private const string CausingCardKey = "WanjianqifaCausingCard";
        ...
        Dictionary<string, object> intermediateResults)
    {
        ...
        var damage = new DamageDescriptor(
            ...
            Reason: "Wanjianqifa",
            CausingCard: causingCard  // The Wanjian Qifa card that causes the damage
        );
        ...
        var nullifiableEffect = NullificationHelper.CreateNullifiableEffect(
            effectKey: $"Wanjianqifa.TargetStep_{target.Seat}",
            targetPlayer: target,
            causingCard: causingCard,
            isNullifiable: true);

        var nullificationResultKey = $"WanjianqifaNullification_{target.Seat}";
...
internal sealed class WanjianqifaTargetHandlerResolver : IResolver
{
    ...
    public ResolutionResult Resolve(ResolutionContext context)
    {
        ...
        var nullificationResultKey = $"WanjianqifaNullification_{_pendingDamage.TargetSeat}";
            ...
            nullificationResult.IsNullified)
        {
            ...
            if (context.LogSink is not null)
            {
                ...
                {
                    EventType = "WanjianqifaNullified",
                    Level = "Info",
                    Message = $"Wanjian Qifa effect on player {_pendingDamage.TargetSeat} was nullified",
                    Data = new
                    {
