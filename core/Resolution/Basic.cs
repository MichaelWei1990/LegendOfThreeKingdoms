...
public sealed class UseCardResolver : IResolver
{
        ...
        out List<Card> selectedCards)
    {
        ...
        // Extract selected cards from hand, equipment zone, or CardCandidates
        // Also check CardCandidates for virtual cards (e.g., generated by skills like Lijian)
        var handCards = sourcePlayer.HandZone.Cards?.ToList() ?? new List<Card>();
        var equipmentCards = sourcePlayer.EquipmentZone.Cards?.ToList() ?? new List<Card>();
        var cardCandidates = action.CardCandidates?.ToList() ?? new List<Card>();
        
        selectedCards = selectedCardIds
            .Select(id =>
            {
                // First try to find in hand cards
                var handCard = handCards.FirstOrDefault(c => c.Id == id);
                if (handCard is not null)
                    return handCard;
                
                // If not in hand, try to find in equipment zone
                var equipmentCard = equipmentCards.FirstOrDefault(c => c.Id == id);
                if (equipmentCard is not null)
                    return equipmentCard;
                
                // If not in hand or equipment, try to find in CardCandidates (for virtual cards)
                // But only if the card is actually marked as virtual or is a skill-generated card
                var candidateCard = cardCandidates.FirstOrDefault(c => c.Id == id);
                if (candidateCard is not null)
                {
                    // For non-virtual cards, they must exist in hand or equipment zone
                    // CardCandidates should only be used for virtual cards
                    // If card is in CardCandidates but not in hand/equipment, it's an error
                    // unless it's explicitly marked as virtual (which will be checked later in MoveCardsIfNeeded)
        ...
        // Additional validation: if card is only in CardCandidates but not in hand/equipment,
        // it must be a virtual card. For regular cards, they must exist in physical zones.
        // This check happens before MoveCardsIfNeeded to provide a better error message.
        foreach (var card in selectedCards)
        {
            var inHand = handCards.Any(c => c.Id == card.Id);
            var inEquipment = equipmentCards.Any(c => c.Id == card.Id);
            
            if (!inHand && !inEquipment)
            {
                // Card is only in CardCandidates, not in physical zones
                // This is only valid for virtual cards (e.g., generated by skills like Lijian)
                // For regular cards that should be in hand/equipment, this is an error
                // We'll allow it to pass here and let MoveCardsIfNeeded check for virtual card flag
                // If it's not a virtual card, MoveCardsIfNeeded will fail and we'll handle it there
            }
        }
    ...
    private static IResolver? CreateSpecificResolver(Card actualCard)
    {
        return actualCard.CardType switch
        {
            ...
            CardType.Trick => actualCard.CardSubType switch
            {
                ...
                CardSubType.Harvest => new ImmediateTrickResolver(),
                CardSubType.JieDaoShaRen => new JieDaoShaRenResolver(),
        ...
        List<Card> selectedCards)
    {
        ...
        // Check if this is a virtual card (generated by skills like Lijian)
        // Virtual cards don't exist in physical zones, so we skip moving them
        var intermediateResults = conversionResult.UpdatedIntermediateResults ?? context.IntermediateResults;
        if (intermediateResults is not null && 
            intermediateResults.TryGetValue("IsVirtualCard", out var isVirtualObj) &&
            isVirtualObj is bool isVirtual && isVirtual)
        {
            // Virtual cards don't need to be moved - they don't exist in physical zones
            return ResolutionResult.SuccessResult;
        }
        ...
        if (moveResult.ShouldMove && moveResult.MoveBeforeResolution && moveResult.CardsToMove is not null)
        {
            try
            {
                // Check if any cards are from equipment zone (for skills like Wusheng)
                var cardsToMove = moveResult.CardsToMove.ToList();
                var handCards = new List<Card>();
                var equipmentCards = new List<Card>();

                foreach (var card in cardsToMove)
                {
                    // Check if card is in hand zone
                    if (sourcePlayer.HandZone.Cards.Any(c => c.Id == card.Id))
                    {
                        handCards.Add(card);
                    }
                    // Check if card is in equipment zone
                    else if (sourcePlayer.EquipmentZone.Cards.Any(c => c.Id == card.Id))
                    {
                    ...
                    else
                    {
                        ...
                        // Check if it's a virtual card (already checked earlier)
                        // If not virtual, this is an error - card doesn't exist in physical zones
                        // This should have been caught earlier, but we'll handle it here
                        // by checking if the card is marked as virtual
                        var isCardVirtual = intermediateResults is not null &&
                                           intermediateResults.TryGetValue("IsVirtualCard", out var isCardVirtualObj) &&
                                           isCardVirtualObj is bool isCardVirtualFlag &&
                                           isCardVirtualFlag;
                        
                        if (!isCardVirtual)
                        {
                            // Card is not in physical zones and not virtual - this is an error
                            return ResolutionResult.Failure(
                                ResolutionErrorCode.CardNotFound,
                                messageKey: "resolution.usecard.cardNotFound",
                                details: new { CardId = card.Id, Message = "Card not found in hand or equipment zone" });
                        }
                        
                        // Virtual cards don't need to be moved - skip them
                    }
                }
                ...
                // Move equipment cards to discard pile
                if (equipmentCards.Count > 0 && sourcePlayer.EquipmentZone is Model.Zones.Zone equipmentZone)
                {
            ...
            {
                // Check if the exception is about card not being in source zone
                // This indicates the card doesn't exist in the expected physical zone
                if (ex is InvalidOperationException && 
                    (ex.Message.Contains("does not belong to the specified source zone") ||
                     ex.Message.Contains("Card with Id=")))
                {
                    return ResolutionResult.Failure(
                        ResolutionErrorCode.CardNotFound,
                        messageKey: "resolution.usecard.cardNotFound",
...
internal sealed class CardConversionCleanupResolver : IResolver
{
    ...
    public ResolutionResult Resolve(ResolutionContext context)
    {
        ...
        // Find the original card in the source player's hand or equipment zone
        var cardInHand = sourcePlayer.HandZone.Cards?.FirstOrDefault(c => c.Id == _originalCard.Id);
        var cardInEquipment = sourcePlayer.EquipmentZone.Cards?.FirstOrDefault(c => c.Id == _originalCard.Id);

        if (cardInHand is null && cardInEquipment is null)
        {
        ...
        try
        {
            ...
            if (cardInHand is not null)
            {
                ...
                else if (cardInEquipment is not null && sourcePlayer.EquipmentZone is Model.Zones.Zone equipmentZone)
            {
                // Move equipment card to discard pile
                var moveDescriptor = new CardMoveDescriptor(
                    SourceZone: equipmentZone,
                    TargetZone: game.DiscardPile,
                    Cards: new[] { cardInEquipment },
                    Reason: CardMoveReason.Discard,
                    Ordering: CardMoveOrdering.ToTop,
                    Game: game
                );
