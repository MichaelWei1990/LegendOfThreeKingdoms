...
public sealed class DefensiveHorseTests
{
    ...
    private static Card CreateDefensiveHorseCard(int id = 1, string definitionId = "defensive_horse")
    {
        return new Card
        {
            Id = id,
    ...
    /// <summary>
    /// Tests that EquipmentSkillRegistry can register and retrieve equipment skills.
    /// Input: Empty registry, DefensiveHorseSkillFactory, equipment ID "defensive_horse".
    /// Expected: After registration, GetSkillForEquipment returns a skill with correct Id and SkillType.Locked.
    /// </summary>
    [TestMethod]
    public void EquipmentSkillRegistryRegisterEquipmentSkillCanRetrieveSkill()
    {
        // Arrange
        ...
        // Act
        registry.RegisterEquipmentSkill("defensive_horse", factory);
        var skill = registry.GetSkillForEquipment("defensive_horse");
    ...
    /// <summary>
    /// Tests that EquipmentSkillRegistry prevents duplicate equipment skill registrations.
    /// Input: Registry with "defensive_horse" already registered, attempting to register same ID again.
    /// Expected: ArgumentException is thrown when trying to register duplicate equipment ID.
    /// </summary>
    [TestMethod]
    public void EquipmentSkillRegistryRegisterEquipmentSkillWithDuplicateIdThrowsException()
    {
        // Arrange
        var registry = new EquipmentSkillRegistry();
        ...
        // Act
        registry.RegisterEquipmentSkill("defensive_horse", factory1);

        // Assert
        Assert.ThrowsException<ArgumentException>(() =>
            registry.RegisterEquipmentSkill("defensive_horse", factory2));
    }

    /// <summary>
    /// Tests that EquipmentSkillRegistry returns null for unregistered equipment IDs.
    /// Input: Empty registry, querying for "nonexistent" equipment ID.
    /// Expected: GetSkillForEquipment returns null when equipment ID is not registered.
    /// </summary>
    [TestMethod]
    public void EquipmentSkillRegistryGetSkillForEquipmentWithInvalidIdReturnsNull()
    {
        // Arrange
        var registry = new EquipmentSkillRegistry();

        // Act
        var skill = registry.GetSkillForEquipment("nonexistent");
    ...
    public void EquipResolverEquipDefensiveHorseWhenAlreadyEquippedReplacesOldEquipment()
    {
        ...
        var game = CreateDefaultGame(2);
        var player = game.Players[0];
        var oldHorse = CreateDefensiveHorseCard(1, "defensive_horse_old");
        var newHorse = CreateDefensiveHorseCard(2, "defensive_horse_new");
    ...
    public void RangeRuleServiceWithDefensiveHorseIncreasesAttackDistanceRequirement()
    {
        ...
        equipmentSkillRegistry.RegisterEquipmentSkill("defensive_horse", new DefensiveHorseSkillFactory());
        ...
        var defensiveHorseSkill = equipmentSkillRegistry.GetSkillForEquipment("defensive_horse");
